...
public class Animator {
	...
	public void update() {
		...
		Map<String, Matrix4> currentPose = calculateCurrentAnimationPose();
		applyPoseToJoints(currentPose, entity.getRootJoint(), new Matrix4());
	}
	 ...
	 *            the pose.
	 */
	private void applyPoseToJoints(Map<String, Matrix4> currentPose, Joint joint, Matrix4 parentTransform) {
		Matrix4 currentLocalTransform = currentPose.get(joint.name);
		Matrix4 currentTransform = new Matrix4(parentTransform).mult(currentLocalTransform);
		for (Joint childJoint : joint.children) {
			applyPoseToJoints(currentPose, childJoint, currentTransform);
		}
		currentTransform.mult(joint.getInverseBindTransform());
