...
public class Joint {
	...
	private Matrix4 animatedTransform = new Matrix4(); // Transformation from original position to current pose in model space (so from the origin of the world)
	
	private final Matrix4 localBindTransform;							//local refers to parent joint, bind refers to original pose of the model
	private Matrix4 inverseBindTransform = new Matrix4(); // is the localBindTransform but in model space (from this joint location/rotation to world origin)
	...
	protected void calcInverseBindTransform(Matrix4 parentBindTransform) {
		Matrix4 bindTransform = new Matrix4(parentBindTransform).mult(localBindTransform);
		inverseBindTransform = new Matrix4(bindTransform).inverse();
